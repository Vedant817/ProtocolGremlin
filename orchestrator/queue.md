...
- ~~Memory Protection Unit (MPU) & Multi-Tenant Partitioning Engine~~ -
  done: `docs/mpu_study.md` (spatial memory partitioning theory, IO pin mask protection, temporal cycle budget enforcement, fault classification matrix, zero-silicon software sandboxing, and synthesizable hardware MPU macro PPA scaling on IHP 130nm SG13G2), `tools/mpu_model.py` (`AccessType`, `PrivilegeLevel`, `MpuFaultType`, `MemoryRegion`, `MpuControllerModel`, `MpuPpaModel`, `build_mpu_sandbox_supervisor_asm`, `build_mpu_illegal_write_trap_asm`, `build_mpu_io_permission_violation_asm`, `build_mpu_temporal_budget_enforcement_asm`) + `test/test_mpu.py` (6/6 cocotb tests pass: authorized tenant execution in partition yielding to supervisor R2=0x00, out-of-bounds pointer write trapped R2=0xEE, IO pin authorization violation trapped R2=0xEA with pin drive suppressed, temporal cycle budget preemption R2=0xEB, cycle-accurate hardware MPU model and PPA scaling, and quarantine High-Z pin electrical safety), `MUT_38` killed.
