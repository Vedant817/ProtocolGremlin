...
- **What exists:**
  ...
  5. **Firmware & Decoders:** Parameterized bit-banged UART TX and zero-jitter UART RX with
     ...
     and Wiegand Security Access Control Protocol Reader/Writer & Pulse Width Discovery Engine (`docs/wiegand_study.md`, `tools/wiegand_model.py`) supporting 26-bit standard framing (H10301), active-low pulsed signaling on DATA0 and DATA1, leading even / trailing odd parity structures, single-cycle WAITEDGE pulse width and bit interval discovery, physical line short / tamper detection, and synthesizable peripheral macro scaling (+285 cells, +1.48% area overhead),
     paired with independent `UartReceiver`, `UartTransmitter`, `SpiSlave`, `I2cSlave`, `OneWireSlave`, `PS2Device`, `JtagTarget`, `SwdTarget`, `ManchesterDecoder`, `CanReceiverModel`, `Dmx512ReceiverModel`, `AutobaudTransmitterModel`, `HdlcTransmitter`, `HdlcReceiver`, `TrafficGenerator`, `DualCoreSystem`, `UsbReceiver`, `ProtocolFuzzer`, `EthernetTransceiverModel`, `WatchdogModel`, `PowerModel`, `CryptoPerformanceModel`, `SchedulerModel`, `CanFdReceiver`, `SsoGroundBounceModel`, `IrDropModel`, `CrossTalkModel`, `InterruptControllerModel`, `InterruptPpaModel`, `MpuControllerModel`, `MpuPpaModel`, `CrcCoprocessorModel`, `CrcPpaModel`, `I3cTargetDevice`, `I3cPpaModel`, `QeiEncoder`, `QeiDecoder`, `QeiPpaModel`, `LinSlaveModel`, `SsiEncoderModel`, `BissEncoderModel`, `BissPpaModel`, `Mil1553RemoteTerminal`, `Mil1553PpaModel`, `WiegandReaderModel`, and `WiegandPpaModel` verification models.
  6. **Mutation Testing:** Standalone harness (`scripts/mutate.py`) testing 45
     architectural fault categories, measuring **100.0% kill rate (45/45 killed)**
...
- **What's verified:** 221/221 RTL tests pass via `scripts/regress.sh` and 8/8 gate-level timing tests pass via `scripts/test_gl.sh`:
...
docs/           architecture, ISA, verification, toolchain, PPA, limitations, multilane_study, ethernet_study, watchdog_study, power_study, crypto_study, scheduler_study, canfd_study, floorplan_study, interrupt_study, mpu_study, crc_study, i3c_study, qei_study, lin_study, biss_study, mil1553_study, wiegand_study
...
bash scripts/regress.sh     # runs all 221 cocotb regression tests (~65s)
...
python3 scripts/mutate.py   # runs RTL mutation testing campaign (45/45 killed)
...
- High-Speed Synchronous Serial Interface (SSI / BiSS-C) Absolute Rotary Encoder Engine completed: Point-to-point master-slave synchronous clock generation (MA on pin 3), serial data sampling (SLO on pin 4), ALU-based reflected binary (Gray code) to standard binary decoding, BiSS-C Single-Cycle Data framing (Ack, Start, CDS, Position, nE error, nW warning), and polynomial CRC-6 integrity verification ($P(x) = x^6 + x + 1$) verified on the 8-bit core with 0 silicon gates, while a dedicated synthesizable peripheral macro (+466 cells, +2.44% area overhead) supports clock rates >780 MHz on IHP 130nm SG13G2.
- MIL-STD-1553B Avionic Multiplex Data Bus Dual-Redundant Protocol Engine completed: 3-bit non-Manchester synchronization waveforms (Command/Status Sync and Data Sync spanning 24 clock cycles), 16-bit Manchester II Biphase-L information decoding/encoding, odd parity generation and checking, Remote Terminal address filtering (5-bit RT address), and automatic Bus A to Bus B dual-redundant failover on response timeout verified on the 8-bit core with 0 silicon gates, while a dedicated dual-channel synthesizable peripheral macro (+486 cells, +2.55% area overhead) supports bus frequencies >750 MHz on IHP 130nm SG13G2.
- Wiegand Security Access Control Protocol Reader/Writer & Pulse Width Discovery Engine completed: 26-bit standard framing (H10301), active-low pulsed signaling on DATA0 and DATA1 with initial idle line settling, leading even / trailing odd parity structures, single-cycle WAITEDGE pulse width and bit interval discovery, and physical line short / tamper detection (DATA0=0 and DATA1=0 trapped with R2=0xAA) verified on the 8-bit core with 0 silicon gates, while a dedicated synthesizable peripheral macro (+285 cells, +1.48% area overhead) supports clock rates >810 MHz on IHP 130nm SG13G2.

## What to work on next

Full prioritized backlog: `orchestrator/queue.md`. Entering continuous loop:

1. Iteration 43: ARINC 429 Mark 33 Digital Information Transfer System (DITS) Avionic Protocol Engine.
2. Iteration 44: MIDI 2.0 (Universal MIDI Packet - UMP) High-Resolution Synthesizer & Control Protocol Engine.
3. Iteration 45: I2S (Inter-IC Sound) & TDM Digital Audio Multi-Channel Serial Interface Engine.
4. Iteration 46: SpaceWire (ECSS-E-ST-50-52C) Data-Strobe (DS) Spacecraft Serial Bus Protocol Engine.
