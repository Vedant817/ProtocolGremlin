...
- **What exists:**
  ...
  5. **Firmware & Decoders:** Parameterized bit-banged UART TX and zero-jitter UART RX with
     ...
     and Local Interconnect Network (LIN v2.2A / ISO 17987) Automotive Protocol Engine (`docs/lin_study.md`, `tools/lin_model.py`) supporting master Break/Sync generation (Break >= 13 bits low, Sync 0x55), single-cycle slave Break duration capture via WAITEDGE, PID mixed parity generation/checking, Classic and Enhanced ones' complement carry-wrap checksums, and synthesizable coprocessor macro scaling (+351 cells, +1.84% area overhead),
     paired with independent `UartReceiver`, `UartTransmitter`, `SpiSlave`, `I2cSlave`, `OneWireSlave`, `PS2Device`, `JtagTarget`, `SwdTarget`, `ManchesterDecoder`, `CanReceiverModel`, `Dmx512ReceiverModel`, `AutobaudTransmitterModel`, `HdlcTransmitter`, `HdlcReceiver`, `TrafficGenerator`, `DualCoreSystem`, `UsbReceiver`, `ProtocolFuzzer`, `EthernetTransceiverModel`, `WatchdogModel`, `PowerModel`, `CryptoPerformanceModel`, `SchedulerModel`, `CanFdReceiver`, `SsoGroundBounceModel`, `IrDropModel`, `CrossTalkModel`, `InterruptControllerModel`, `InterruptPpaModel`, `MpuControllerModel`, `MpuPpaModel`, `CrcCoprocessorModel`, `CrcPpaModel`, `I3cTargetDevice`, `I3cPpaModel`, `QeiEncoder`, `QeiDecoder`, `QeiPpaModel`, and `LinSlaveModel` verification models.
  6. **Mutation Testing:** Standalone harness (`scripts/mutate.py`) testing 42
     architectural fault categories, measuring **100.0% kill rate (42/42 killed)**
...
- **What's verified:** 203/203 RTL tests pass via `scripts/regress.sh` and 8/8 gate-level timing tests pass via `scripts/test_gl.sh`:
...
docs/           architecture, ISA, verification, toolchain, PPA, limitations, multilane_study, ethernet_study, watchdog_study, power_study, crypto_study, scheduler_study, canfd_study, floorplan_study, interrupt_study, mpu_study, crc_study, i3c_study, qei_study, lin_study
...
bash scripts/regress.sh     # runs all 203 cocotb regression tests (~65s)
...
python3 scripts/mutate.py   # runs RTL mutation testing campaign (42/42 killed)
