...
- **What exists:**
  ...
  8. **Gate-Level Timing Verification:** Full post-synthesis physical netlist simulation
     ...
     verifying 8/8 physical protocol tests across external chip pins in 25.41s.
...
- **What's verified:** 287/287 RTL tests pass via `scripts/regress.sh` and 8/8 gate-level timing tests pass via `scripts/test_gl.sh`:
...
bash scripts/regress.sh     # runs all 287 cocotb regression tests (~80s)
bash scripts/test_gl.sh     # runs gate-level timing simulation (8/8 tests pass, ~25s)
sby -f formal/core.sby      # runs SymbiYosys formal verification with Z3 (20 steps pass)
python3 scripts/mutate.py   # runs RTL mutation testing campaign (56/56 killed)
...
**Iteration 54 Next:** IEEE 1588 PTP (Precision Time Protocol / IEEE 1588-2019 / IEC 61588) Hardware Timestamping & Sub-Microsecond Clock Synchronization Engine (PTP message parsing: Sync, Follow_Up, Delay_Req, Delay_Resp; hardware ingress/egress timestamp capture via WAITEDGE timestamp mode; syntonization/frequency ratio drift compensation; Mean Path Delay and Master-to-Slave Offset estimation; PPA scaling on IHP 130nm SG13G2).
