...
- **What exists:**
  ...
  5. **Firmware & Decoders:** Parameterized bit-banged UART TX and zero-jitter UART RX with
     ...
     and Universal Multi-Protocol Bus Bridging Matrix Engine (`tools/bridge_matrix_model.py`) realizing I2C-to-SPI, UART-to-CAN with ACK monitoring, 1-Wire-to-UART, and multi-byte continuous stream translations with framing error isolation,
  ...
  6. **Mutation Testing:** Standalone harness (`scripts/mutate.py`) testing 30
     architectural fault categories, measuring **100.0% kill rate (30/30 killed)**
...
- **What's verified:** 132/132 RTL tests pass via `scripts/regress.sh` and 8/8 gate-level timing tests pass via `scripts/test_gl.sh`:
...
bash scripts/regress.sh     # runs all 132 cocotb regression tests (~60s)
bash scripts/test_gl.sh     # runs gate-level timing simulation (8/8 tests pass, ~27s)
sby -f formal/core.sby      # runs SymbiYosys formal verification with Z3 (20 steps pass)
python3 scripts/mutate.py   # runs RTL mutation testing campaign (30/30 killed)
...
1. Iteration 28: Hardware Watchdog Timer & Brownout Recovery Circuit Feasibility Study.
2. Iteration 29: Dynamic Power & Energy Optimization Study (Clock Gating & Instruction Micro-Architectural Profiling).
3. Iteration 30: Cryptographic Accelerator Feasibility Study (ChaCha8 / Poly1305 / SHA-256 bit-sliced microcode).
