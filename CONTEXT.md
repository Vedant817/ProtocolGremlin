...
- **What's verified:** 251/251 RTL tests pass via `scripts/regress.sh` and 8/8 gate-level timing tests pass via `scripts/test_gl.sh`:
...
docs/           architecture, ISA, verification, toolchain, PPA, limitations, multilane_study, ethernet_study, watchdog_study, power_study, crypto_study, scheduler_study, canfd_study, floorplan_study, interrupt_study, mpu_study, crc_study, i3c_study, qei_study, lin_study, biss_study, mil1553_study, wiegand_study, arinc429_study, midi2_study, i2s_study, spacewire_study, sent_study
...
bash scripts/regress.sh     # runs all 251 cocotb regression tests (~75s)
...
python3 scripts/mutate.py   # runs RTL mutation testing campaign (50/50 killed)
...
- I2S (Inter-IC Sound) & TDM Digital Audio Multi-Channel Serial Interface Engine completed: Continuous SCK bit clocking, Word Select (WS/LRCLK) with 1-bit standard delay, two's-complement MSB-first serial PCM audio transmission and reception, in-register digital volume attenuation via ALU shift operations (6 dB attenuation step), TDM multi-channel frame synchronization and slot extraction (extracting target slot 2 from an 8-slot frame) verified on the 8-bit core with 0 silicon gates, while a dedicated synthesizable audio coprocessor macro (+428 cells, +2.22% area overhead) supports bit clock frequencies >780 MHz on IHP 130nm SG13G2.
- SpaceWire (ECSS-E-ST-50-52C) Data-Strobe Spacecraft Serial Bus Protocol Engine completed: Data-Strobe (DS) physical line coding ($\Delta D \oplus \Delta S = 1$), clock recovery without PLL ($\text{CLK} = \Delta D \oplus \Delta S$), 4-bit Control Characters (FCT, EOP, EEP, ESC), 10-bit Data Characters with odd character parity ($P = 1 \oplus \text{popcount}(D) \pmod 2$), composite tokens (NULL keepalive, Time-Code synchronization), credit-based flow control accounting (+8 credits per FCT), and in-register parity error trapping verified on the 8-bit core with 0 silicon gates, while a dedicated synthesizable spacecraft coprocessor macro (+456 cells, +2.37% area overhead) supports data rates >760 Mbps on IHP 130nm SG13G2.
...
**Iteration 48 Next:** EtherCAT (Ethernet for Control Automation Technology - IEC 61158) Sub-Datagram Processing & "Processing-on-the-Fly" Engine (EtherCAT slave frame header parsing, sub-datagram command extraction [APRD, APWR, FPRD, FPWR, BRD, BWR], working counter [WKC] dynamic incrementation on the fly, 16-bit CRC/FCS validation, dual-port forwarding with sub-microsecond latency, and IHP 130nm SG13G2 PPA scaling).
