...
- **What exists:**
  ...
  8. **Gate-Level Timing Verification:** Full post-synthesis physical netlist simulation
     ...
     verifying 8/8 physical protocol tests across external chip pins in 31.10s.
...
- **What's verified:** 449/449 RTL tests pass via `scripts/regress.sh` and 8/8 gate-level timing tests pass via `scripts/test_gl.sh`:
...
- AXI4/AXI5 Memory-Mapped (AXI4-MM) On-Chip Interconnect & Burst Controller Engine completed: ARM AMBA 4 AXI4 (ARM IHI 0022E) and AMBA 5 AXI5 (ARM IHI 0022H) specifications, 5 independent decoupled channels (Read Address AR, Read Data R, Write Address AW, Write Data W, Write Response B), AXI5 atomic transactions (Compare-and-Swap, Swap, Atomic Add, Atomic Bitwise), burst types (FIXED 0x00, INCR 0x01, WRAP 0x02), mathematical address generation with aligned wrap boundary calculations, response status codes (OKAY 0x00, EXOKAY 0x01, SLVERR 0x02, DECERR 0x03), channel opcodes (AR_REQ 0x01, R_DATA 0x02, AW_REQ 0x03, W_DATA 0x04, B_RESP 0x05, ATOMIC_REQ 0x06, SYNC_SOF 0xA5, IDLE 0x7E), in-register channel filtering with fault trapping of invalid commands (0x7F trapped with R2=0xEE), in-register outstanding transaction credit tracking (increment on Response completed, decrement on Request issued, and underflow fault trapping with R2=0xEE), 16-bit CCITT CRC protection (polynomial 0x1021), hardware bit-serial MSB-first SHIFTOUT beat transmission, WAITEDGE sync SOF edge synchronization, and receiver interconnect link lock FSM verified on the 8-bit core with 0 silicon gates, while a dedicated synthesizable AXI4/AXI5 crossbar and burst controller macro (+630 cells, +3.27% area overhead, 800.0 MHz max frequency, 61.50 uW at 10 MHz) delivers 32000.0 Mbps raw interconnect throughput at 0.00077 pJ/bit on IHP 130nm SG13G2.
- AXI4-Stream & TileLink On-Chip Streaming Fabric & Interconnect Engine completed: AMBA AXI4-Stream (ARM IHI 0051B) and SiFive TileLink v1.8.1 specifications, decoupled TVALID/TREADY handshaking, TLAST packet framing, TKEEP byte qualifiers, TileLink Channel A/D request-response credit accounting, TileLink opcodes (PUT_FULL_DATA 0x00, PUT_PARTIAL_DATA 0x01, ARITHMETIC_DATA 0x02, LOGICAL_DATA 0x03, GET 0x04, INTENT 0x05, ACCESS_ACK 0x06, ACCESS_ACK_DATA 0x07, SYNC_SOF 0xA5, IDLE 0x7E), in-register opcode filtering with fault trapping of invalid opcodes (0x7F trapped with R2=0xEE), in-register TileLink credit tracking (increment on AccessAck, decrement on Request beat, and underflow fault trapping with R2=0xEE), 16-bit CCITT CRC protection (polynomial 0x1021), hardware bit-serial MSB-first SHIFTOUT beat transmission, WAITEDGE sync SOF edge synchronization, and receiver link lock FSM verified on the 8-bit core with 0 silicon gates, while a dedicated synthesizable AXI-Stream/TileLink macro (+625 cells, +3.24% area overhead, 800.0 MHz max frequency, 61.00 uW at 10 MHz) delivers 25600.0 Mbps raw throughput at 0.00076 pJ/bit on IHP 130nm SG13G2.
- NVLink (NVIDIA High-Speed GPU Interconnect) Physical & Data Link Layer Engine completed: NVLink 1.0 to 5.0 specifications, sub-link architecture, remote memory transactions, buffer credit flow control accounting, link training sync comma delimiters (SYNC 0xBC comma 0b10111100, IDLE 0x7E), NVLink opcode multiplexing (READ_REQ 0x01, READ_RESP 0x02, WRITE_REQ 0x03, ATOMIC_REQ 0x04, FLOW_CTRL_CREDIT 0x05, LINK_TRAIN_REQ 0x06), in-register opcode filtering with fault trapping of invalid opcodes (0x7F trapped with R2=0xEE), in-register sub-link buffer credit tracking (increment on Credit Return, decrement on Packet Send, and underflow fault trapping with R2=0xEE), 16-bit CCITT CRC protection (polynomial 0x1021), hardware bit-serial MSB-first SHIFTOUT packet transmission, WAITEDGE sync comma edge synchronization, and receiver link lock FSM verified on the 8-bit core with 0 silicon gates, while a dedicated synthesizable NVLink macro (+620 cells, +3.21% area overhead, 800.0 MHz max frequency, 60.50 uW at 10 MHz) delivers 20000.0 Mbps raw throughput per lane at 0.00075 pJ/bit on IHP 130nm SG13G2.
...
- InfiniBand HDR/NDR Physical & Link Layer Engine completed: Link training sequences TS1 (`0x4A`, 'J') and TS2 (`0x45`, 'E'), Start-of-Packet (`0xFB`) and End-of-Packet (`0xFD`) delimiters, Base Transport Header (BTH) OpCode filtering with in-register verification (`0x00`, `0x04`, `0x0A`, `0x11` match; `0x3F` trapped with `R2=0xEE`), dual-layer CRC protection (16-bit Variant CRC VCRC with polynomial 0x1021 and 32-bit Invariant CRC ICRC with Ethernet polynomial 0xEDB88320), WAITEDGE SOP rising-edge synchronization, in-register VCRC slice validation, and receiver link lock FSM verified on the 8-bit core with 0 silicon gates, while a dedicated synthesizable InfiniBand HDR/NDR PCS/Link macro (+590 cells, +3.06% area overhead, 800.0 MHz max frequency, 57.50 uW at 10 MHz) delivers 50000.0 Mbps raw throughput per lane (HDR) or 100000.0 Mbps (NDR) at 0.00115 pJ/bit on IHP 130nm SG13G2.
- Fibre Channel 32G/64G (FC-FS-5) Physical Layer Engine completed: ANSI INCITS 545 FC-FS-5 specification, 32G/64G line rates, PAM4 and 64b/66b line coding, Ordered Set primitives (IDLE 0x7E, R_RDY 0x4B, NOS 0x55, OLS 0x33, LR 0x0F, LRR 0xF0), buffer-to-buffer credit (BB_Credit) flow control with credit tracking and underflow trapping (R2=0xEE), SOF delimiter in-register filtering (SOFn matching, illegal delimiter trapped with R2=0xEE), frame encapsulation with 32-bit FC-CRC32 and receiver BB_Credit accounting verified on the 8-bit core with 0 silicon gates, while a dedicated synthesizable Fibre Channel PCS macro (+595 cells, +3.08% area overhead, 800.0 MHz max frequency, 58.00 uW at 10 MHz) delivers 32000.0/64000.0 Mbps raw throughput per lane at 0.00181 pJ/bit on IHP 130nm SG13G2.
- Coherent Accelerator Processor Interface (OpenCAPI / CXL) Physical Layer Engine completed: CXL 1.1/2.0/3.0/3.1 and OpenCAPI 3.0/4.0 specifications, 32.0/64.0 GT/s PCIe Gen 5/6 electrical PHY, CXL.io / CXL.cache / CXL.mem sub-protocol multiplexing, link sync comma delimiters (SYNC 0xBC, IDLE 0x7E), in-register protocol classification with fault trapping of invalid protocol IDs (0x1F trapped with R2=0xEE), 16-bit FLIT CRC protection (polynomial x^16 + x^12 + x^5 + 1 / 0x1021), in-register CRC-16 syndrome slice validation, WAITEDGE sync comma edge synchronization, and receiver link lock FSM verified on the 8-bit core with 0 silicon gates, while a dedicated synthesizable CXL/OpenCAPI PCS/Link macro (+600 cells, +3.11% area overhead, 800.0 MHz max frequency, 58.50 uW at 10 MHz) delivers 32000.0/64000.0 Mbps raw throughput per lane at 0.00183 pJ/bit on IHP 130nm SG13G2.
- HyperTransport 3.1 Physical Layer & Link Protocol Engine completed: HyperTransport 1.0/2.0/3.0/3.1 specifications, 3.2 GHz DDR signaling, CAD/CTL command multiplexing (NOP 0x00, READ_REQ 0x20, WRITE_REQ 0x40, RESPONSE 0xC0), link training sync comma delimiters (SYNC 0xBC K28.5 comma, IDLE 0x7E), in-register command classification with fault trapping of invalid commands (0x7F trapped with R2=0xEE), in-register Virtual Channel buffer credit flow control accounting (increment on Credit Return, decrement on Packet Send, and underflow fault trapping with R2=0xEE), 32-bit packet CRC protection (polynomial 0xEDB88320), WAITEDGE sync comma edge synchronization, and receiver link lock FSM verified on the 8-bit core with 0 silicon gates, while a dedicated synthesizable HyperTransport PCS/Link macro (+605 cells, +3.14% area overhead, 800.0 MHz max frequency, 59.00 uW at 10 MHz) delivers 51200.0 Mbps raw throughput per link at 0.00115 pJ/bit on IHP 130nm SG13G2.
- InfiniBand XDR/GDR & Ultra Ethernet Consortium (UEC) Transport Engine completed: UEC 1.0 specification, InfiniBand XDR 800G / GDR 1.6T physical signaling, packet multipath spraying, selective packet retransmission, congestion control, transport opcode multiplexing (RDMA_WRITE 0x10, RDMA_READ_REQ 0x20, RDMA_READ_RESP 0x30, CONGESTION_NOTIF 0x40, SELECTIVE_ACK 0x50), link training sync comma delimiters (SYNC 0xBC K28.5 comma, IDLE 0x7E), in-register opcode filtering with fault trapping of invalid opcodes (0x7F trapped with R2=0xEE), in-register Congestion Window (CWND) flow control accounting (increment on ACK, decrement on congestion, and underflow fault trapping with R2=0xEE), 32-bit packet CRC protection (polynomial 0xEDB88320), WAITEDGE sync comma edge synchronization, and receiver link lock FSM verified on the 8-bit core with 0 silicon gates, while a dedicated synthesizable UEC/XDR Transport macro (+610 cells, +3.17% area overhead, 800.0 MHz max frequency, 59.50 uW at 10 MHz) delivers 200000.0 Mbps raw throughput per link at 0.00030 pJ/bit on IHP 130nm SG13G2.
- Universal Chiplet Interconnect Express (UCIe 1.0/2.0) Die-to-Die Physical & Sideband Engine completed: UCIe 1.0 & 2.0 specifications, standard packaging (16 lanes @ 2-32 Gbps) vs advanced packaging (64 lanes @ up to 64 Gbps), dedicated 800 MHz single-ended sideband serial link, sideband opcode multiplexing (REG_READ_REQ 0x01, REG_READ_RESP 0x02, REG_WRITE 0x03, LINK_TRAIN_REQ 0x04, LANE_REPAIR_MAP 0x05, POWER_STATE_REQ 0x06), link training sync comma delimiters (SYNC 0xBC K28.5 comma, IDLE 0x7E), in-register opcode filtering with fault trapping of invalid opcodes (0x7F trapped with R2=0xEE), in-register spare lane allocation flow accounting (decrement on Fault Repaired, increment on Spare Restored, and underflow fault trapping with R2=0xEE), 16-bit sideband CCITT CRC protection (polynomial 0x1021), hardware bit-serial SHIFTOUT packet transmission, WAITEDGE sync comma edge synchronization, and receiver link lock FSM verified on the 8-bit core with 0 silicon gates, while a dedicated synthesizable UCIe D2D Sideband macro (+615 cells, +3.19% area overhead, 800.0 MHz max frequency, 60.00 uW at 10 MHz) delivers 32000.0 Mbps raw throughput at 0.00094 pJ/bit on IHP 130nm SG13G2.
- Bunch of Wires (BoW / OpenHBI - OCP ODSA) Die-to-Die Physical Layer Engine completed: OCP ODSA BoW specification, BoW-Base (standard organic substrate, 16 wires @ 2-16 Gbps) vs BoW-Fast (advanced packaging), OpenHBI bus interface mapping, forwarded strobe/clock deskew, slice wire sparing and lane repair, BoW opcode multiplexing (CALIB_REQ 0x01, CALIB_RESP 0x02, TRAIN_STROBE_REQ 0x03, DATA_TRANSFER 0x04, LANE_REMAP 0x05, POWER_DOWN_REQ 0x06), link training sync comma delimiters (SYNC 0xBC comma 0b10111100, IDLE 0x7E), in-register opcode filtering with fault trapping of invalid opcodes (0x7F trapped with R2=0xEE), in-register slice spare wire allocation flow accounting (decrement on Lane Remap, increment on Spare Restore, and underflow fault trapping with R2=0xEE), 16-bit ANSI/IBM CRC-16 protection (polynomial 0x8005), hardware bit-serial MSB-first SHIFTOUT packet transmission, WAITEDGE sync comma edge synchronization, and receiver link lock FSM verified on the 8-bit core with 0 silicon gates, while a dedicated synthesizable BoW/OpenHBI slice macro (+610 cells, +3.17% area overhead, 800.0 MHz max frequency, 59.00 uW at 10 MHz) delivers 32000.0 Mbps raw throughput at 0.00045 pJ/bit on IHP 130nm SG13G2.
- NVLink (NVIDIA High-Speed GPU Interconnect) Physical & Data Link Layer Engine completed: NVLink 1.0 to 5.0 specifications, sub-link architecture, remote memory transactions, buffer credit flow control, Data Link opcode multiplexing (READ_REQ 0x01, READ_RESP 0x02, WRITE_REQ 0x03, ATOMIC_REQ 0x04, FLOW_CTRL_CREDIT 0x05, LINK_TRAIN_REQ 0x06), link training sync comma delimiters (SYNC 0xBC K28.5 comma, IDLE 0x7E), in-register opcode filtering with fault trapping of invalid opcodes (0x7F trapped with R2=0xEE), in-register sub-link buffer flow control credit accounting (increment on Credit Return, decrement on Packet Send, and underflow fault trapping with R2=0xEE), 16-bit CCITT CRC protection (polynomial 0x1021), hardware bit-serial MSB-first SHIFTOUT packet transmission, WAITEDGE sync comma edge synchronization, and receiver link lock FSM verified on the 8-bit core with 0 silicon gates, while a dedicated synthesizable NVLink macro (+620 cells, +3.21% area overhead, 800.0 MHz max frequency, 60.50 uW at 10 MHz) delivers 20000.0 Mbps raw throughput per lane at 0.00075 pJ/bit on IHP 130nm SG13G2.
- AXI4-Stream & TileLink On-Chip Streaming Fabric & Interconnect Engine completed: ARM AMBA 4 AXI4-Stream & SiFive TileLink v1.8.1 specifications, TVALID/TREADY handshakes, Channel A-E architecture, credit flow control, TileLink channel opcode multiplexing (PUT_FULL_DATA 0x00, PUT_PARTIAL_DATA 0x01, ARITHMETIC_DATA 0x02, LOGICAL_DATA 0x03, GET 0x04, INTENT 0x05, ACCESS_ACK 0x06, ACCESS_ACK_DATA 0x07), streaming delimiters (SYNC_SOF 0xA5, IDLE 0x7E), in-register opcode filtering with fault trapping of invalid opcodes (0x7F trapped with R2=0xEE), in-register TileLink request-response credit accounting (increment on Response ACK, decrement on Request Send, and underflow fault trapping with R2=0xEE), 16-bit CCITT CRC protection (polynomial 0x1021), hardware bit-serial MSB-first SHIFTOUT packet transmission, WAITEDGE SYNC_SOF delimiter edge synchronization, and receiver streaming lock FSM verified on the 8-bit core with 0 silicon gates, while a dedicated synthesizable AXI4-Stream / TileLink interconnect macro (+625 cells, +3.24% area overhead, 800.0 MHz max frequency, 61.00 uW at 10 MHz) delivers 10000.0 Mbps raw streaming throughput at 0.00076 pJ/bit on IHP 130nm SG13G2.
- AXI4/AXI5 Memory-Mapped (AXI4-MM) On-Chip Interconnect & Burst Controller Engine completed: ARM AMBA 4 AXI4 & AMBA 5 AXI5 specifications, five independent decoupled channels (AR, R, AW, W, B), burst address generation (FIXED 0x00, INCR 0x01, WRAP 0x02 with wrap boundary calculation), response status codes (OKAY 0x00, EXOKAY 0x01, SLVERR 0x02, DECERR 0x03), channel command multiplexing (AR_REQ 0x01, R_DATA 0x02, AW_REQ 0x03, W_DATA 0x04, B_RESP 0x05, ATOMIC_REQ 0x06), in-register channel filtering with fault trapping of invalid commands (0x7F trapped with R2=0xEE), in-register outstanding transaction credit accounting (increment on Response completed, decrement on Request issued, and underflow fault trapping with R2=0xEE), 16-bit CCITT CRC protection (polynomial 0x1021), hardware bit-serial MSB-first SHIFTOUT packet transmission, WAITEDGE SYNC_SOF delimiter edge synchronization, and receiver interconnect link lock FSM verified on the 8-bit core with 0 silicon gates, while a dedicated synthesizable AXI4/AXI5 crossbar and burst controller macro (+630 cells, +3.27% area overhead, 800.0 MHz max frequency, 61.50 uW at 10 MHz) delivers 32000.0 Mbps raw interconnect throughput at 0.00077 pJ/bit on IHP 130nm SG13G2.
- AMBA AHB-Lite / APB4 Multi-Master Interconnect & Low-Power Peripheral Subsystem Engine completed: ARM AMBA 3 AHB-Lite pipelined bus architecture (decoupled Address and Data phases), burst address calculation (SINGLE, INCR, WRAP4/8/16, INCR4/8/16 with aligned boundary wrapping), APB4 low-power peripheral bus FSM (IDLE, SETUP, ACCESS) with PREADY wait states, PSLVERR error signaling, and PSTRB byte strobes, AHB-to-APB bridge translation and HREADY wait state insertion, in-register command filtering matching valid opcodes (0x01..0x06) and trapping illegal commands (0x7F trapped with R2=0xEE), in-register bridge buffer credit flow control tracking (increment on APB complete, decrement on AHB dispatch, and underflow trapped with R2=0xEE), 16-bit CCITT CRC protection (0x1021), hardware bit-serial MSB-first SHIFTOUT packet transmission, WAITEDGE sync rising-edge synchronization, and receiver interconnect link lock FSM verified on the 8-bit core with 0 silicon gates, while a dedicated synthesizable AHB-Lite / APB4 bridge macro (+635 cells, +3.29% area overhead, 800.0 MHz max frequency, 62.00 uW at 10 MHz) delivers 25600.0 Mbps raw interconnect throughput at 0.00097 pJ/bit on IHP 130nm SG13G2.
- Wishbone B4 & Avalon-MM On-Chip Interconnect & Pipelined Crossbar Engine completed: Wishbone B4 (OpenCores/FOSSi) specification (classic single/block cycles vs pipelined registered feedback cycles with STALL_I backpressure handshaking) and Intel/Altera Avalon Memory-Mapped (Avalon-MM) specification (waitrequest backpressure, readdatavalid pipelined reads, burstcount incremental bursts, and response status codes OKAY, RESERVED, SLAVEERROR, DECODEERROR), crossbar bridge translation between Wishbone and Avalon protocols, in-register command filtering matching valid opcodes (0x01..0x06) and trapping illegal commands (0x7F trapped with R2=0xEE), in-register crossbar buffer credit flow control tracking (increment on slave ACK, decrement on master STB, and underflow trapped with R2=0xEE), 16-bit CCITT CRC protection (0x1021), hardware bit-serial MSB-first SHIFTOUT packet transmission, WAITEDGE sync rising-edge synchronization, and receiver interconnect link lock FSM verified on the 8-bit core with 0 silicon gates, while a dedicated synthesizable Wishbone B4 / Avalon-MM pipelined crossbar macro (+640 cells, +3.32% area overhead, 800.0 MHz max frequency, 62.50 uW at 10 MHz) delivers 25600.0 Mbps raw interconnect throughput at 0.00098 pJ/bit on IHP 130nm SG13G2.
- ARM AMBA CHI & ACE Cache-Coherent Interconnect Engine completed: ARM AMBA CHI (Issues B/C/D/E) and AMBA 4 ACE specifications, packetized flit channels (REQ, RSP, DAT, SNP) replacing parallel buses for high-core-count fabrics, discrete node hierarchy (RN-F, RN-D, RN-I, HN-F, HN-I, SN-F/I), 5-state MOESI cache coherency model (Invalid 0x00, Unique Clean 0x01, Unique Dirty 0x02, Shared Clean 0x03, Shared Dirty 0x04) with deterministic state transition logic, in-register opcode filtering matching valid opcodes (0x01..0x08) and trapping illegal commands (0x7F trapped with R2=0xEE), in-register coherent transaction credit flow control tracking (increment on CompAck returned, decrement on request issued, and underflow trapped with R2=0xEE), 16-bit CCITT CRC protection (0x1021), hardware bit-serial MSB-first SHIFTOUT packet transmission, WAITEDGE sync rising-edge synchronization, and receiver snoop filter directory tracking & link lock FSM verified on the 8-bit core with 0 silicon gates, while a dedicated synthesizable ARM AMBA CHI / ACE Home Node (HN-F) / Coherent Interconnect macro (+645 cells, +3.34% area overhead, 800.0 MHz max frequency, 63.00 uW at 10 MHz) delivers 32000.0 Mbps raw interconnect throughput at 0.00098 pJ/bit on IHP 130nm SG13G2.

## What to work on next

Full prioritized backlog: `orchestrator/queue.md`.
**Iteration 81 Next:** HBM3 / HBM3e (IEEE 2445) High-Bandwidth Memory Physical Layer & Command Engine.
