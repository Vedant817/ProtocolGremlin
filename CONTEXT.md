...
- **What exists:**
  ...
  8. **Gate-Level Timing Verification:** Full post-synthesis physical netlist simulation
     ...
     verifying 8/8 physical protocol tests across external chip pins in 23.49s.
...
- **What's verified:** 425/425 RTL tests pass via `scripts/regress.sh` and 8/8 gate-level timing tests pass via `scripts/test_gl.sh`:
...
bash scripts/regress.sh     # runs all 425 cocotb regression tests (~125s)
...
python3 scripts/mutate.py   # runs RTL mutation testing campaign (79/79 killed)
...
**Iteration 77 Next:** AXI4/AXI5 Memory-Mapped (AXI4-MM) On-Chip Interconnect & Burst Controller Engine.
