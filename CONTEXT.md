...
- **What exists:**
  ...
  8. **Gate-Level Timing Verification:** Full post-synthesis physical netlist simulation
     ...
     verifying 8/8 physical protocol tests across external chip pins in 26.69s.
...
- **What's verified:** 275/275 RTL tests pass via `scripts/regress.sh` and 8/8 gate-level timing tests pass via `scripts/test_gl.sh`:
...
bash scripts/regress.sh     # runs all 275 cocotb regression tests (~75s)
...
python3 scripts/mutate.py   # runs RTL mutation testing campaign (54/54 killed)
...
**Iteration 51 Next:** Modbus RTU / ASCII (IEC 61158 / Modbus-IDA) Protocol Engine & Serial Bus Master/Slave Controller (binary framing with 3.5 character inter-frame silence discovery via WAITEDGE, 16-bit CRC-16/MODBUS polynomial 0xA001, ASCII colon/LRC/CRLF framing, function code handling 0x03/0x06/0x10, and IHP 130nm SG13G2 PPA scaling).
