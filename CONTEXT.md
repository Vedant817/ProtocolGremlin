...
- **What exists:**
  ...
  6. **Mutation Testing:** Standalone harness (`scripts/mutate.py`) testing 65
     architectural fault categories, measuring **100.0% kill rate (65/65 killed)**
  ...
  8. **Gate-Level Timing Verification:** Full post-synthesis physical netlist simulation
     ...
     verifying 8/8 physical protocol tests across external chip pins in 23.45s.
...
- **What's verified:** 341/341 RTL tests pass via `scripts/regress.sh` and 8/8 gate-level timing tests pass via `scripts/test_gl.sh`:
...
**Iteration 63 Next:** MIPI D-PHY v2.5 Physical Layer Engine (High-Speed DDR up to 4.5 Gbps, Low-Power Escape Mode, SoT/EoT sync sequence, Spaced-One-Hot line encoding, and IHP 130nm SG13G2 PPA scaling).
