...
- **What exists:**
  ...
  8. **Gate-Level Timing Verification:** Full post-synthesis physical netlist simulation
     ...
     verifying 8/8 physical protocol tests across external chip pins in 24.08s.
...
- **What's verified:** 419/419 RTL tests pass via `scripts/regress.sh` and 8/8 gate-level timing tests pass via `scripts/test_gl.sh`:
...
bash scripts/regress.sh     # runs all 419 cocotb regression tests (~125s)
bash scripts/test_gl.sh     # runs gate-level timing simulation (8/8 tests pass, ~24s)
sby -f formal/core.sby      # runs SymbiYosys formal verification with Z3 (20 steps pass)
python3 scripts/mutate.py   # runs RTL mutation testing campaign (78/78 killed)
...
**Iteration 76 Next:** AXI4-Stream & TileLink On-Chip Streaming Fabric & Interconnect Engine.
