...
- **What exists:**
  ...
  8. **Gate-Level Timing Verification:** Full post-synthesis physical netlist simulation
     ...
     verifying 8/8 physical protocol tests across external chip pins in 24.67s.
...
- **What's verified:** 281/281 RTL tests pass via `scripts/regress.sh` and 8/8 gate-level timing tests pass via `scripts/test_gl.sh`:
...
bash scripts/regress.sh     # runs all 281 cocotb regression tests (~75s)
...
python3 scripts/mutate.py   # runs RTL mutation testing campaign (55/55 killed)
