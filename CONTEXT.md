...
- **What exists:**
  ...
  8. **Gate-Level Timing Verification:** Full post-synthesis physical netlist simulation
     ...
     verifying 8/8 physical protocol tests across external chip pins in 22.69s.
...
- **What's verified:** 299/299 RTL tests pass via `scripts/regress.sh` and 8/8 gate-level timing tests pass via `scripts/test_gl.sh`:
...
**Iteration 56 Next:** USB 2.0 Full-Speed (12 Mbps) NRZI, Dynamic Bit Stuffing/Destuffing, and PID Packet Engine (differential D+/D- signaling, SOP/EOP delimiters, SYNC field 0x80, token/data/handshake PID complement verification, CRC-5/CRC-16 validation, and IHP 130nm SG13G2 PPA scaling).
