...
- **What exists:**
  ...
  8. **Gate-Level Timing Verification:** Full post-synthesis physical netlist simulation
     ...
     verifying 8/8 physical protocol tests across external chip pins in 24.74s.
...
- **What's verified:** 323/323 RTL tests pass via `scripts/regress.sh` and 8/8 gate-level timing tests pass via `scripts/test_gl.sh`:
...
bash scripts/regress.sh     # runs all 299 cocotb regression tests (~118s)
...
python3 scripts/mutate.py   # runs RTL mutation testing campaign (58/58 killed)
