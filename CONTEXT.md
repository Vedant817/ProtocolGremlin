...
- **What's verified:** 239/239 RTL tests pass via `scripts/regress.sh` and 8/8 gate-level timing tests pass via `scripts/test_gl.sh`:
...
bash scripts/regress.sh     # runs all 239 cocotb regression tests (~75s)
...
python3 scripts/mutate.py   # runs RTL mutation testing campaign (48/48 killed)
...
1. Iteration 46: SpaceWire (ECSS-E-ST-50-52C) Data-Strobe (DS) Spacecraft Serial Bus Protocol Engine.
