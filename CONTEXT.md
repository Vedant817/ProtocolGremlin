...
**CI / tapeout status (2026-09-19, commit `1771322`):** All required GitHub Actions workflows are green. `info.yaml` tiles are **6x4** (Jane Street / IHP CMOS5L maximum). GitHub Pages source is **GitHub Actions**. Verified: docs success; RTL `TESTS=509 PASS=509`; GDS harden success (~1h48m); Tiny Tapeout precheck success; gate-level `GATES=yes` `TESTS=8 PASS=8` (PDK `sg13cmos5l_udp.v` included); viewer success. Run: https://github.com/Vedant817/ProtocolGremlin/actions/runs/35468093583
