...
- **What exists:**
  ...
  5. **Firmware & Decoders:** Parameterized bit-banged UART TX and zero-jitter UART RX with
     ...
     and PCI Express Base Gen 1 (2.5 GT/s) Physical Layer & 8b/10b Link Engine (`docs/pcie_gen1_study.md`, `tools/pcie_gen1_model.py`) supporting ANSI X3.230 DC-balanced 8b/10b transmission block coding (bounded run-length <= 5), continuous running disparity invariant tracking (RD- and RD+), standard PCIe control symbols (K28.5 COM, K28.1 SKP, K28.2 FTS, K28.3 IDL, K23.7 PAD, K27.7 STP, K29.7 SDP, K30.7 SDP), Ordered Sets (TS1, TS2, SKP ordered sets compensating +/-300 ppm clock drift, FTS ordered sets for sub-us L0s exit, EIOS electrical idle), 16-bit maximal-length LFSR data scrambler/descrambler ($G(x) = x^{16} + x^5 + x^4 + x^3 + 1$, seed 0xFFFF), comma delimiter rising-edge synchronization via WAITEDGE, in-register FTS symbol validation, and synthesizable PCIe Gen 1 PCS macro scaling (+545 cells, +2.83% area overhead, 1000.0 Mbps throughput at 0.0530 pJ/bit),
  ...
  8. **Gate-Level Timing Verification:** Full post-synthesis physical netlist simulation
     ...
     verifying 8/8 physical protocol tests across external chip pins in 25.66s.
...
- **What's verified:** 455/455 RTL tests pass via `scripts/regress.sh` and 8/8 gate-level timing tests pass via `scripts/test_gl.sh`:
...
**Iteration 82 Next:** DDR5 / LPDDR5 (JEDEC JESD79-5 / JESD209-5) Physical Layer & Command Scheduler Engine.
