...
"""
test/test_ddr5.py - Cocotb test suite for DDR5 / LPDDR5 (JEDEC JESD79-5 / JESD209-5)
High-Speed Memory Physical Layer & Command Engine.

Verifies:
1. test_ddr5_master_packet_transmission: Master transmits SYNC_SOF delimiter (0xA5)
...
2. test_ddr5_rx_beat_ingress: Slave synchronizes to SYNC_SOF rising edge on pin 3 via WAITEDGE,
   samples command byte MSB-first into R0 and preserves in R1 (0x05 RD) with status R2 = 0x00.
3. test_ddr5_command_filter_and_fault_trapping: In-register validation of DDR5 commands
   (valid 0x01 ACT, 0x02 PRE, 0x03 REF, 0x04 PDE, 0x05 RD, 0x06 WR, 0x07 MPC, 0x08 MRW -> R2 = 0x00;
    illegal 0x7F trapped with R2 = 0xEE).
4. test_ddr5_credit_tracking_and_underflow_trapping: In-register validation of command buffer
   credit increment (ACK/PRE returned), decrement (command dispatched), and underflow prevention (credits=0 -> R2 = 0xEE).
5. test_ddr5_packet_framing_banks_and_receiver: Packet encapsulation, subchannel/bank group/bank state transitions,
   CCITT CRC-16 (0x1021) protection, and Ddr5ReceiverModel link lock state machine.
6. test_ddr5_standards_and_ppa: Bank states, command opcodes, CRC-16 determinism,
...
from ddr5_model import (  # noqa: E402
    Ddr5BankState,
    Ddr5OpCode,
    compute_ddr5_crc16,
    encode_ddr5_packet,
    ...
    Ddr5ReceiverModel,
    Ddr5PpaModel,
    build_ddr5_tx_beat_asm,
    build_ddr5_rx_beat_asm,
    build_ddr5_command_filter_asm,
    build_ddr5_credit_tracker_asm,
)
...
@cocotb.test()
async def test_ddr5_master_packet_transmission(dut):
    """
    Test 1: Master DDR5 Packet Header Transmission:
    ...
    pin_tx = 3
    baud_cycles = 4
    expected_sync = int(Ddr5OpCode.SYNC_SOF)    # 0xA5 (0b10100101)
    expected_opcode = int(Ddr5OpCode.ACT)       # 0x01
    expected_addr = 0x80                        # Target Row Address 0x80

    asm_code = build_ddr5_tx_beat_asm(
...
@cocotb.test()
async def test_ddr5_rx_beat_ingress(dut):
    """
    Test 2: Slave DDR5 SYNC_SOF Ingress & Command Sampling:
    ...
    pin_rx = 3
    baud_cycles = 4
    sync_delimiter = int(Ddr5OpCode.SYNC_SOF)   # 0xA5 = 0b10100101
    test_command = int(Ddr5OpCode.RD)          # 0x05 = 0b00000101

    asm_code = build_ddr5_rx_beat_asm(
        pin_rx=pin_rx,
        baud_cycles=baud_cycles,
    )
    words = assemble("\n".join(asm_code))
...
@cocotb.test()
async def test_ddr5_command_filter_and_fault_trapping(dut):
    """
    Test 3: In-Register DDR5 Command Filter & Fault Trapping:
    ...
    for cmd_val, name in valid_test_cases:
        asm_code = build_ddr5_command_filter_asm(cmd_val)
        words = assemble("\n".join(asm_code))
        await _init_dut_and_bootload(dut, words)
    ...
    illegal_cmd = 0x7F
    asm_code = build_ddr5_command_filter_asm(illegal_cmd)
    words = assemble("\n".join(asm_code))
    await _init_dut_and_bootload(dut, words)
...
@cocotb.test()
async def test_ddr5_credit_tracking_and_underflow_trapping(dut):
    """
    Test 4: In-Register DDR5 Command Buffer Credit Tracking & Underflow Trapping:
    ...
    asm_code_return = build_ddr5_credit_tracker_asm(event_type=1, initial_credits=4)
    ...
    asm_code_send = build_ddr5_credit_tracker_asm(event_type=2, initial_credits=4)
    words_send = assemble("\n".join(asm_code_send))
    ...
    asm_code_underflow = build_ddr5_credit_tracker_asm(event_type=2, initial_credits=0)
...
@cocotb.test()
async def test_ddr5_packet_framing_banks_and_receiver(dut):
    """
    Test 5: DDR5 Framing, Subchannel/Bank Group/Bank State Transitions & Receiver Model:
    - Tests packet encapsulation, CRC-16 generation, and decoding.
    - Tests CRC corrupt packet detection.
    - Tests Ddr5ReceiverModel bank state transitions (IDLE -> ACTIVE -> IDLE) and link lock acquisition.
    """
    clock = Clock(dut.clk, 100, unit="ns")
    cocotb.start_soon(clock.start())

    # Packet Encoding & Decoding Tests
    pkt = encode_ddr5_packet(
        opcode=Ddr5OpCode.ACT,
        subch_id=1,
        bank_group=3,
        bank_id=2,
        addr=0x1234,
    ...
    decoded = decode_ddr5_packet(frame_bytes)

    assert decoded is not None, "Valid DDR5 packet failed decoding"
    assert decoded["opcode"] == Ddr5OpCode.ACT, f"Expected ACT, got {decoded['opcode']}"
    assert decoded["subch_id"] == 1, f"Expected subch_id=1, got {decoded['subch_id']}"
    ...
    decoded_corrupt = decode_ddr5_packet(bytes(corrupted_bytes))
    assert decoded_corrupt is None, "Corrupted packet incorrectly passed decoding"

    # Receiver Model Link Lock and Bank State Test
    receiver = Ddr5ReceiverModel(num_subchannels=2, bank_groups_per_subch=8, banks_per_group=4)
    ...
    # Process ACT command: Subchannel 0, BG 1, Bank 2
    act_pkt = encode_ddr5_packet(
        opcode=Ddr5OpCode.ACT,
        subch_id=0,
        bank_group=1,
        bank_id=2,
    ...
    assert receiver.get_bank_state(0, 1, 2) == Ddr5BankState.ACTIVE, "Bank state should be ACTIVE after ACT"
    ...
    # Process PRE command: Subchannel 0, BG 1, Bank 2
    pre_pkt = encode_ddr5_packet(
        opcode=Ddr5OpCode.PRE,
        subch_id=0,
        bank_group=1,
        bank_id=2,
        addr=0x0000,
    ...
    assert receiver.get_bank_state(0, 1, 2) == Ddr5BankState.IDLE, "Bank state should be IDLE after PRE"
...
@cocotb.test()
async def test_ddr5_standards_and_ppa(dut):
    """
    Test 6: Standards Compliance and PPA Model Validation:
    - Verifies DDR5 bank states and command opcodes.
    ...
    # Bank states
    assert Ddr5BankState.IDLE == 0x00
    assert Ddr5BankState.ACTIVE == 0x01
    assert Ddr5BankState.PRECHARGING == 0x02
    assert Ddr5BankState.REFRESHING == 0x03

    # Command opcodes
    assert Ddr5OpCode.NOP == 0x00
    assert Ddr5OpCode.ACT == 0x01
    assert Ddr5OpCode.PRE == 0x02
    assert Ddr5OpCode.REF == 0x03
    assert Ddr5OpCode.PDE == 0x04
    assert Ddr5OpCode.RD == 0x05
    assert Ddr5OpCode.WR == 0x06
    assert Ddr5OpCode.MPC == 0x07
    assert Ddr5OpCode.MRW == 0x08
    assert Ddr5OpCode.IDLE == 0x7E
    assert Ddr5OpCode.SYNC_SOF == 0xA5

    # CRC-16 determinism test
    test_data = b"DDR5 / LPDDR5 JEDEC JESD79-5 Command & Physical Layer Engine"
    crc1 = compute_ddr5_crc16(test_data)
    crc2 = compute_ddr5_crc16(test_data)
    ...
    ppa = Ddr5PpaModel.get_metrics()
    assert ppa["macro_cells"] == 655, f"PPA macro cells mismatch: {ppa['macro_cells']}"
    assert ppa["macro_ge"] == 1285.0, f"PPA GE mismatch: {ppa['macro_ge']}"
    assert ppa["macro_area_um2"] == 4815.0, f"PPA area mismatch: {ppa['macro_area_um2']}"
    assert ppa["f_max_mhz"] == 800.0, f"PPA f_max mismatch: {ppa['f_max_mhz']}"
    assert ppa["nominal_power_uw_10mhz"] == 64.00, f"PPA power mismatch: {ppa['nominal_power_uw_10mhz']}"
    assert ppa["raw_throughput_mbps"] == 38400.0, f"PPA throughput mismatch: {ppa['raw_throughput_mbps']}"
    assert ppa["energy_pj_per_bit"] == 0.00096, f"PPA energy mismatch: {ppa['energy_pj_per_bit']}"
