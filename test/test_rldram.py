...
"""
test/test_rldram.py - Cocotb test suite for RLDRAM 3 (Reduced Latency DRAM 3 / Micron Technology)
Synchronous Memory Engine.

Verifies:
1. test_rldram_master_packet_transmission: Master transmits SYNC_SOF delimiter (0xA5)
...
2. test_rldram_rx_beat_ingress: Slave synchronizes to SYNC_SOF rising edge on pin 3 via WAITEDGE,
   samples CMD byte MSB-first into R0 and preserves in R1 (0x03 AREF) with status R2 = 0x00.
3. test_rldram_command_filter_and_fault_trapping: In-register validation of RLDRAM 3 command opcodes
   (valid 0x00 NOP, 0x01 READ, 0x02 WRITE, 0x03 AREF, 0x04 MRS, 0x05 ZQCL -> R2 = 0x00;
    illegal 0x7F trapped with R2 = 0xEE).
4. test_rldram_credit_tracking_and_underflow_trapping: In-register validation of RLDRAM buffer credit
   increment (Event 1), decrement (Event 2), and underflow prevention (credits=0 -> R2 = 0xEE).
5. test_rldram_packet_framing_banks_and_receiver: Packet encapsulation, 16 target banks,
   CCITT CRC-16 (0x1021) protection, and RldramReceiverModel link lock state machine.
6. test_rldram_standards_and_ppa: Bank states, 16 Bank IDs, OpCodes, CRC-16 determinism,
...
from rldram_model import (  # noqa: E402
    RldramBankState,
    RldramBankId,
    RldramOpCode,
    compute_rldram_crc16,
    encode_rldram_packet,
    decode_rldram_packet,
    RldramReceiverModel,
    RldramPpaModel,
    build_rldram_tx_beat_asm,
    build_rldram_rx_beat_asm,
    build_rldram_command_filter_asm,
    build_rldram_credit_tracker_asm,
)
...
@cocotb.test()
async def test_rldram_master_packet_transmission(dut):
    """
    Test 1: Master RLDRAM Packet Header Transmission:
    ...
    pin_tx = 3
    baud_cycles = 4
    expected_sync = int(RldramOpCode.SYNC_SOF)       # 0xA5 (0b10100101)
    expected_opcode = int(RldramOpCode.READ)         # 0x01
    expected_bank = int(RldramBankId.BANK_0)         # 0x00

    asm_code = build_rldram_tx_beat_asm(
...
@cocotb.test()
async def test_rldram_rx_beat_ingress(dut):
    """
    Test 2: Slave RLDRAM SYNC Synchronization & Command Ingress:
    Drives SYNC_SOF (0xA5) on pin 3, WAITEDGE rising edge detects bit 7,
    strides to CMD byte midpoint, ingresses 8 bits MSB-first (0x03 AREF) into R0,
    ...
    pin_rx = 3
    baud_cycles = 4
    command_to_send = int(RldramOpCode.AREF)  # 0x03 AREF

    asm_code = build_rldram_rx_beat_asm(
        pin_rx=pin_rx,
        baud_cycles=baud_cycles,
    )
    words = assemble("\n".join(asm_code))
    ...
    sync_val = int(RldramOpCode.SYNC_SOF)
...
@cocotb.test()
async def test_rldram_command_filter_and_fault_trapping(dut):
    """
    Test 3: In-Register RLDRAM 3 Command Filtering and Fault Trapping:
    Verifies valid commands:
      0x00 (NOP), 0x01 (READ), 0x02 (WRITE), 0x03 (AREF), 0x04 (MRS), 0x05 (ZQCL)
    ...
    valid_commands = [
        int(RldramOpCode.NOP),
        int(RldramOpCode.READ),
        int(RldramOpCode.WRITE),
        int(RldramOpCode.AREF),
        int(RldramOpCode.MRS),
        int(RldramOpCode.ZQCL),
    ]
    ...
    for op in valid_commands:
        asm_code = build_rldram_command_filter_asm(test_opcode=op)
        words = assemble("\n".join(asm_code))
        await _init_dut_and_bootload(dut, words)
    ...
    illegal_op = 0x7F
    asm_code = build_rldram_command_filter_asm(test_opcode=illegal_op)
    words = assemble("\n".join(asm_code))
    await _init_dut_and_bootload(dut, words)
...
@cocotb.test()
async def test_rldram_credit_tracking_and_underflow_trapping(dut):
    """
    Test 4: In-Register RLDRAM Memory Buffer Credit Tracking and Underflow Trapping:
    Verifies:
    ...
    asm_inc = build_rldram_credit_tracker_asm(initial_credits=4, event_type=1)
    words_inc = assemble("\n".join(asm_inc))
    ...
    asm_dec = build_rldram_credit_tracker_asm(initial_credits=4, event_type=2)
    words_dec = assemble("\n".join(asm_dec))
    ...
    asm_underflow = build_rldram_credit_tracker_asm(initial_credits=0, event_type=2)
...
@cocotb.test()
async def test_rldram_packet_framing_banks_and_receiver(dut):
    """
    Test 5: RLDRAM 3 Packet Encapsulation, 16 Banks, and Receiver Model:
    Verifies:
      1. Packet encoding and decoding across 16 banks with 16-bit CCITT CRC.
      2. Corrupted CRC rejection.
      3. Bank state transitions (IDLE -> READY -> ACTIVE_READ -> ACTIVE_WRITE -> AUTO_REFRESH -> READY).
      4. Credit accounting in Python receiver model.
      5. Link lock FSM acquisition after 4 consecutive valid frames.
    """
    receiver = RldramReceiverModel(num_banks=16, initial_credits=4)
    assert not receiver.link_locked, "Receiver should initially be unlocked"

    # Frame 1: NOP to Bank 0
    pkt1 = encode_rldram_packet(bank=0, opcode=RldramOpCode.NOP, addr=0x00)
    res1 = receiver.process_packet(pkt1)
    assert res1["valid"], f"Packet 1 failed: {res1.get('error')}"

    # Frame 2: READ to Bank 5
    pkt2 = encode_rldram_packet(bank=5, opcode=RldramOpCode.READ, addr=0x10, payload=b"\x00\x00")
    res2 = receiver.process_packet(pkt2)
    assert res2["valid"], f"Packet 2 failed: {res2.get('error')}"
    assert receiver.get_bank_state(5) == RldramBankState.ACTIVE_READ
    assert receiver.credits == 3, f"Expected 3 credits remaining, got {receiver.credits}"

    # Frame 3: WRITE to Bank 10
    pkt3 = encode_rldram_packet(bank=10, opcode=RldramOpCode.WRITE, addr=0x20, payload=b"\xAA\x55")
    res3 = receiver.process_packet(pkt3)
    assert res3["valid"], f"Packet 3 failed: {res3.get('error')}"
    assert receiver.get_bank_state(10) == RldramBankState.ACTIVE_WRITE
    assert receiver.credits == 2

    # Frame 4: AREF (Auto Refresh) to Bank 15
    pkt4 = encode_rldram_packet(bank=15, opcode=RldramOpCode.AREF, addr=0x00)
    res4 = receiver.process_packet(pkt4)
    assert res4["valid"], f"Packet 4 failed: {res4.get('error')}"
    assert receiver.get_bank_state(15) == RldramBankState.AUTO_REFRESH
    assert receiver.credits == 1
    assert receiver.link_locked, "Receiver should be link-locked after 4 valid frames"

    # Frame 5: MRS to Bank 0
    pkt5 = encode_rldram_packet(bank=0, opcode=RldramOpCode.MRS, addr=0x00)
    res5 = receiver.process_packet(pkt5)
    assert res5["valid"], f"Packet 5 failed: {res5.get('error')}"
    assert receiver.get_bank_state(0) == RldramBankState.READY

    # Return credits
    receiver.return_credit(3)
...
@cocotb.test()
async def test_rldram_standards_and_ppa(dut):
    """
    Test 6: RLDRAM 3 Standards Compliance and Calibrated IHP 130nm SG13G2 PPA Model:
    Verifies:
      1. Bank states, 16 Bank IDs, and OpCode constants.
      2. 16-bit CCITT CRC determinism across test vectors.
      3. Physical PPA scaling model on IHP 130nm SG13G2.
    """
    # Standards verification
    assert RldramBankState.IDLE == 0x00
    assert RldramBankState.READY == 0x01
    assert RldramBankState.ACTIVE_READ == 0x02
    assert RldramBankState.ACTIVE_WRITE == 0x03
    assert RldramBankState.AUTO_REFRESH == 0x04
    assert RldramBankState.ERROR_COLLISION == 0x05

    assert RldramBankId.BANK_0 == 0x00
    assert RldramBankId.BANK_7 == 0x07
    assert RldramBankId.BANK_8 == 0x08
    assert RldramBankId.BANK_15 == 0x0F

    assert RldramOpCode.NOP == 0x00
    assert RldramOpCode.READ == 0x01
    assert RldramOpCode.WRITE == 0x02
    assert RldramOpCode.AREF == 0x03
    assert RldramOpCode.MRS == 0x04
    assert RldramOpCode.ZQCL == 0x05
    assert RldramOpCode.IDLE == 0x7E
    assert RldramOpCode.SYNC_SOF == 0xA5

    # CRC-16 determinism check
    crc1 = compute_rldram_crc16(b"\xA5\x01\x00\x10")
    crc2 = compute_rldram_crc16(b"\xA5\x01\x00\x10")
    ...
    assert summary["hw_macro_cells"] == 640
    assert summary["hw_macro_ge"] == 1260.0
    assert summary["hw_macro_area_um2"] == 4720.0
    assert summary["hw_macro_area_pct"] == 3.32
    assert summary["hw_macro_fmax_mhz"] == 800.00
    assert summary["hw_macro_power_uw_10mhz"] == 63.00
    assert summary["hw_macro_throughput_mbps"] == 34133.3
    assert summary["hw_macro_energy_pj_per_bit"] == 0.00088
